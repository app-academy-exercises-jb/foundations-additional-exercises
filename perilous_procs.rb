...
def some?(arr, &proc)
    found = false
    arr.each { |ele| found = true if proc.call(ele) }
    found
end
...
def exactly?(arr, num, &proc)
    found = []
    arr.each { |ele| found << ele if proc.call(ele) }
    found.length == num
end
...
def filter_out(arr, &proc)
    filtered = []
    arr.each { |ele| filtered << ele unless proc.call(ele) }
    filtered
end
...
def at_least?(arr, num, &proc)
    verified = []
    arr.each { |ele| verified << ele if proc.call(ele) }
    verified.length >= num
end
...
def first_index(arr, &proc)
    ...
    i = 0 #can't use Array.each_with_index
    arr.each { |ele| idx ? idx : (idx = i if proc.call(ele)) ; i += 1 }
...
def xnor_select(arr, proc1, proc2)
    xnored = []
    arr.each { |ele| xnored << ele if (proc1.call(ele) && proc2.call(ele)) || (!proc1.call(ele) && !proc2.call(ele)) }
    xnored
end
...
def multi_map(arr, n=1, &proc)
    n.times { |i| arr.each_with_index { |ele, idx| arr[idx] = proc.call(ele) } }
    arr
end
...
def proctition(arr, &prc)
    ...
    arr.each { |ele| prc.call(ele) ? trues << ele : falsies << ele }
...
def selected_map!(arr, prc1, prc2)
    arr.each_with_index { |ele, idx| prc1.call(ele) ? arr[idx] = prc2.call(ele) : nil }
    nil
end
...
def chain_map(num, proc_arr)
    proc_arr.reduce(num) { |acc, proc| proc.call(acc) }
end
...
def proc_suffix(str, proc_hash)
    ...
    str.split(" ") { |c| 
        word = c
        proc_hash.keys.select { |proc| proc.call(c) == true }.each { |proc| word += proc_hash[proc] }
        new_str << word
    }
    new_str.join(" ")
end
...
def proctition_platinum(arr, *procs)
    proc_hash = Hash.new { |h, k| h[k] = Array.new }
    procs.each_with_index { |proc, idx| 
        proc_hash[idx+1] += arr.select { |ele| proc.call(ele) == true }
        arr.delete_if { |ele| proc.call(ele) == true }
    }
    proc_hash
end
...
def procipher(str, proc_hash)
    str.split(" ").map { |word|
        (proc_hash.keys.any? { |proc| proc.call(word) }) ? 
            (proc_hash.keys.select { |proc| proc.call(word) == true }.reduce(word) { |acc, proc| proc_hash[proc].call(acc) }) :
            word
    }.join(" ")
end
# is_yelled = Proc.new { |s| s[-1] == '!' }
...
def picky_procipher(sentence, proc_hash)
    sentence.split(" ").map { |word| 
        (proc_hash.keys.any? { |proc| proc.call(word) }) ? 
            (proc_hash[proc_hash.keys.select { |proc| proc.call(word) == true }[0]].call(word)) :
            word
    }.join(" ")
end
# is_yelled = Proc.new { |s| s[-1] == '!' }
